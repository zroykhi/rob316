classdef FNSimple2D_Obst < handle
    properties (SetAccess = private)
        ...
        ind_reach_goal      % Iteration of the first goal reach
    ...
    methods
        % class constructor
        function this = FNSimple2D_Obst(rand_seed, max_nodes, map, conf)
...
%             if (rand_token > 0.05)
            ...
            %%%%%%%%
            % TODO : change sampling to take obstacles into account
            % this.obstacle.num is the number of obstacles
            % this.obstacle.output{obs_ind} is an array giving the
            %                                   obstacles corners
%             else
%                 obstacle_num = randi(this.obstacle.num);
%                 current_obstacle = this.obstacle.output{obstacle_num};
%                 size_obstacle = size(current_obstacle);
%                 nb_corners = size_obstacle(1);
%                 corner_num = randi(nb_corners); %ouptut
%                 position = [current_obstacle(corner_num); current_obstacle(corner_num + nb_corners)];
%             end
            x = rand;
            if x < 0.05
        ...
        function [node_index,new_node_position] = nearest(this, new_node_position)
            
            if(new_node_position(1) < this.XY_BOUNDARY(1)) 
                new_node_position(1) = this.XY_BOUNDARY(1);
            end
            if(new_node_position(1) > this.XY_BOUNDARY(2)) 
                new_node_position(1) = this.XY_BOUNDARY(2);
            end
             if(new_node_position(2) < this.XY_BOUNDARY(3)) 
                new_node_position(2) = this.XY_BOUNDARY(3);
            end
            if(new_node_position(2) > this.XY_BOUNDARY(4)) 
                new_node_position(2) = this.XY_BOUNDARY(4);
            end
        ...
        function new_node_ind = insert_node(this, parent_node_ind, new_node_position,iter)
            goal_dist = norm(this.goal_point'-new_node_position);
            if (goal_dist < this.delta_goal_point) && (this.ind_reach_goal == 0)
                this.ind_reach_goal=iter;
            end
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%
        function path_length = getPathLength(this) 
            %%% Find the optimal path to the goal
            % finding all the point which are in the desired region
            distances = zeros(this.nodes_added, 2);
            distances(:, 1) = sum((this.tree(:,1:(this.nodes_added)) - repmat(this.goal_point', 1, this.nodes_added)).^2);
            ...
            if(dist_index ~= 0)
                ...
                distances = sortrows(distances, 1);
                nearest_node_index = distances(1,2);
                path_length = this.cumcost(nearest_node_index);
            else
                disp('NOTICE! Robot cannot reach the goal');
                path_length = 1e6;
            end            
